...
export const useDDSSetStateActionRegistry = (
  ...
  const getDDS = useCallback(
    async <T extends IFluidLoadable>(key: string): Promise<T | undefined> => {
    ...
    useCallback<RegisterDDSSetStateAction>(
      async <T extends IFluidLoadable>(
        ...
        if (actionsMap) {
          ...
          actionsMap.set(componentId, setLocalStateAction);
          registeredDDSSetStateActionMapRef.current.set(uniqueKey, actionsMap);
        }
        try {
  ...
  const unregisterDDSSetStateAction: UnregisterDDSSetStateAction = useCallback(
    (uniqueKey: string, componentId: string) => {
      let actionsMap = registeredDDSSetStateActionMapRef.current.get(uniqueKey);
      if (actionsMap?.has(componentId)) {
  ...
  useEffect(() => {
    ...
    dynamicObjectMap.forEach(
      (
        ...
        const actionMap = registeredDDSSetStateActionMapRef.current.get(key);
        if (handleValue) {
          handleValue
            .get()
            .then((dds) => {
