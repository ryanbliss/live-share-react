...
export function useMediaSynchronizer(
  ...
  /**
   * Dynamically load the EphemeralEvent DDS for the given unique key.
   */
  const { dds: mediaSession } = useDynamicDDS<EphemeralMediaSession>(
    `<EphemeralMediaSession>:${uniqueKey}`,
    EphemeralMediaSession
  );
  ...
  /**
   * Setup change listeners and start `EphemeralMediaSession` if needed
   */
  useEffect(() => {
